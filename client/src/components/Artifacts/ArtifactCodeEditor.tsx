 ...
 * code view never renders. Pointing `paths.vs` at our own copy fixes it for the editor payload:
 * same-origin scripts are covered by `'self'`. A CDN entry would have been strictly worse — it
 * lets a third party execute JS inside the authenticated app.
 *
 * The language workers are a separate matter and DO need a CSP entry: `editorWorkerHost` spawns
 * them from `blob:` URLs, which `'self'` does not cover, so the fallback chain (`worker-src` →
 * `child-src` → `script-src 'self'`) refuses them. The deployment therefore sends
 * `worker-src 'self' blob:`. Without it the editor still renders — tokenization is main-thread —
 * and only suggestions / parameter hints / semantic services in the editable view break, which
